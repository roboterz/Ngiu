...
import com.example.ngiu.functions.ACCOUNT_TYPE_RECEIVABLE
import com.example.ngiu.functions.CATEGORY_INCOME_REIMBURSE
...
//
    fun loadDataToRam(context: Context){
        ...
        listDetail = AppDatabase.getDatabase(context).trans().getTransRecordDetailByReimburseStatus(REIMBURSABLE)
        listAccounts = AppDatabase.getDatabase(context).account().getAccountExceptType(ACCOUNT_TYPE_RECEIVABLE)
